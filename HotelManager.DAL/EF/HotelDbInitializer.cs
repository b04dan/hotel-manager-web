...
namespace HotelManager.DAL.EF
{
    public static class HotelDbInitializer
    {
        ...
        {
            if (!db.Hotels.Any()) GenerateData(db);
        }
        ...
        public static Residence GenerateResidence(HotelRoom room, Client client)
        {
            return new Residence()
            {
                Client = client,
                HotelRoom = room,
                CheckInDate = DateTime.Today.Subtract(TimeSpan.FromDays(GetRand(1, 20))),
                CheckOutDate = GetRand(1, 4) == 1 ? null : DateTime.Today.Subtract(TimeSpan.FromDays(GetRand(1, 10)))
            };
        }
        ...
        public static Worker GenerateWorker(Hotel hotel)
        {
            return new Worker()
            {
                ...
                Working = true,
                WorkdaySalary = GetRand(1000d, 10000),
